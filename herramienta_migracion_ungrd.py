...
def load_master_authors():
    author_map = {}
    master_file = "prelim-lista-de-autores.qmd"
...
def extract_original_affiliations(fpath, current_yaml, current_body):
    ...
    if not affiliations:
        try:
            result = subprocess.run(["git", "show", f"HEAD:{fpath}"], capture_output=True, text=True, encoding="utf-8")
...
def configure_quarto_yml():
    ...
    if not os.path.exists(qpath):
        print("[QuartoConfig] Error: No se encontró _quarto.yml")
        return
...
def process_chapters(author_map):
    chapters = sorted(glob.glob("0[1-9]-*.qmd") + glob.glob("1[0-9]-*.qmd"))
    ...
    for fpath in chapters:
        ...
        # Verificamos si los autores están en formato simple de lista de strings (crudo) o estructurados
        author_block_match = re.search(r'^author:\s*\n((?:\s+-\s*[^\n]+\n)+)', yaml_content, re.MULTILINE)
        
        # Si está en formato crudo (lista simple de strings):
        ...
        for aut in current_authors:
            ...
            # Si es crudo (o no encontramos afiliaciones asociadas), mapeamos usando prelim-lista-de-autores.qmd
            if not final_nums:
...
def purge_cache():
    folders_to_delete = ["_book", ".quarto"]
